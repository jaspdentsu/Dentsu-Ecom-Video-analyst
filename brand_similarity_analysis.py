...
# def run_brand_similarity(search_url, num_products=15, out_dir="outputs"):
#     os.makedirs(out_dir, exist_ok=True)
#     soup = get_soup_from_url(search_url)
#     product_links = extract_top_15_product_links(soup)
...
#         text_vecs = np.vstack([brand_text_embeds[b] for b in brand_names])
#     else:
#         text_vecs = np.zeros((0, 512))  # assuming USE gives 512-dim vectors
...
#         ])
#     else:
#         image_vecs = np.zeros((0, 512))

#     # --- Similarity matrices ---
#     text_sim = cosine_similarity(text_vecs) if len(brand_names) > 0 else np.array([])
#     image_sim = cosine_similarity(image_vecs) if len(brand_names) > 0 else np.array([])

#     # --- Clustering ---
#     if len(brand_names) > 0:
#         n_clusters = min(5, len(brand_names))
#         clusters = KMeans(n_clusters=n_clusters, random_state=42).fit_predict(text_vecs)
#     else:
#         clusters = np.array([])
...
#     summary_df = pd.DataFrame({
#         'Brand': brand_names,
#         'Cluster': clusters if len(clusters) > 0 else [],
#         'Top_Keywords': get_top_keywords([" ".join(brand_texts[b]) for b in brand_names]) if len(brand_names) > 0 else []
#     })
...
#             2. What factors (tone, color, logo usage, etc.) make them credible to be called similar.
#             3. Where they differ the most.
#             4. Provide actionable insight in 3–4 bullet points.
#             """
